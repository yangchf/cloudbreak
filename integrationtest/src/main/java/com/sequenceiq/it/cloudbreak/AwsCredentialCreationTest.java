...
public class AwsCredentialCreationTest extends AbstractCloudbreakIntegrationTest {
    private final static String URL_PATH = "/user/credentials";
    private final static String CREDENTIAL_NAME = "it-aws-cred";
    ...
    public void cleanUp() {
        String credentialId = itContext.getCleanUpParameter(CloudbreakITContextConstants.CREDENTIAL_ID);
        if (credentialId != null) {
            RestUtil.entityPathRequest(itContext.getContextParam(CloudbreakITContextConstants.CLOUDBREAK_SERVER),
                    ...
                    "credentialId", credentialId).delete("/credentials/{credentialId}");
        }
    }
}
