...
public class ClusterCreationTest extends AbstractCloudbreakIntegrationTest {
    ...
    @BeforeMethod
    @Parameters({ "blueprintName", "stackName" })
    public void setContextParameters(@Optional("") String blueprintName, @Optional("") String stackName) {
        if (StringUtils.hasLength(blueprintName)) {
            itContext.putContextParam(CloudbreakITContextConstants.BLUEPRINT_ID, getResourceIdByName("/user/blueprints/{name}", blueprintName));
        }
    ...
    public void testClusterCreation(@Optional("it-cluster") String clusterName, @Optional("false") boolean emailNeeded) {
        ...
        checkCluster(stackId);
    }

    protected void checkCluster(String stackId) {
